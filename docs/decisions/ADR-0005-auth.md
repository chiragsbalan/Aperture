...
- **Mode-agnostic `sign_in`:** login and signup UI both start Google with `intent=sign_in`. After a verified Google identity, the API **logs in** if that Google subject already exists, or **creates** a Google-only identity + profile if not. There is no separate “signup Google” intent that rejects existing accounts, and no “login Google” intent that rejects missing accounts. Optional BFF `return=login|signup` only chooses which guest page shows an OAuth **error** message.
- **No email auto-link:** if Google `email` already belongs to an identity (e.g. password-only) but that identity has no Google credential, `POST /api/v1/auth/google` with `intent=sign_in` returns **409** telling the user to log in then **Link Google** from Account. Linking requires an authenticated `intent=link` call. This is the only intentional hard-fail for “Continue with Google” on guest pages (not “account missing” / “already registered as Google”).
...
- **Username live availability** (signup + settings rename): protected BFF-attested exact check plus optional DIY bloom in the same slice; soft-deleted reclaim deferred. See [ADR-0018](ADR-0018-username-availability-check.md).
